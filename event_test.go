...
func TestPrefilledEventFieldDoesNotAffectHash(t *testing.T) {
	a := On("click", evMsg{Tag: "X"})
	b := On("click", evMsg{Tag: "X", Event: Event{Type: "click", Key: "Enter"}})
	if a.value != b.value {
...
func TestSpliceMultipleHandlersSameEvent(t *testing.T) {
	a := On("keydown", evMsg{Tag: "Save"})
	b := On("keydown", evMsg{Tag: "DraftAutosave"})
