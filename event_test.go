...
// MapNode rewrites the handlers in its subtree to produce To: the harvest
// types as To for the instance, and each event's From passes through f.
func TestMapNodeRewritesHandlers(t *testing.T) {
	btn := Tag("button", On("click", msgFn("go")))(Text("go"))
	_, h := lower(0, MapNode(func(s string) int { return len(s) }, Tag("div")(btn)))
	tbl := typed[int](h)
	if len(tbl) != 1 {
...
// MapNode applies the same rule as an instance: a handler's Msg need only
// implement From.
func TestMapNodeAcceptsImplementing(t *testing.T) {
	hit := func(jsontext.Value) (hitMsg, error) { return hitMsg{7}, nil }
	_, h := lower(0, MapNode(func(m taggedMsg) string { return m.tag() }, Tag("button", On("click", hit))))
...
func TestMapNodeMismatchPanics(t *testing.T) {
	defer func() {
		r := recover()
		if r == nil {
	...
	lower(0, MapNode(func(int) string { return "" }, Tag("button", On("click", msgFn("oops")))))
}

// Nested MapNodes compose inside out: the inner f sees the handler's
// message and the outer f sees the inner's result.
func TestMapNodeNests(t *testing.T) {
	inner := MapNode(func(s string) int { return len(s) }, Tag("button", On("click", msgFn("go"))))
	_, h := lower(0, MapNode(func(n int) string { return strings.Repeat("x", n) }, inner))
	for _, fn := range typed[string](h) {
		got, err := fn(nil)
		if err != nil {
			t.Fatal(err)
...
// MapNode is transparent to addressing: handler keys and element keys are
// those the subtree has on its own, so wrapping a view in MapNode changes
// nothing the client sees.
func TestMapNodeTransparentToAddresses(t *testing.T) {
	build := func() Node {
		return Tag("ul")(
...
// MapNode covers every node of a fragment, not just a single element.
func TestMapNodeFragment(t *testing.T) {
	frag := Fragment(
		Tag("button", On("click", msgFn("a"))),
		Text("between"),
		Tag("button", On("click", msgFn("b"))),
	)
	_, h := lower(0, MapNode(func(s string) int { return len(s) }, frag))
	tbl := typed[int](h)
	if len(tbl) != 2 {
...
func TestMapNodeErrorSkipsF(t *testing.T) {
	fail := func(jsontext.Value) (string, error) { return "", fmt.Errorf("no thanks") }
	_, h := lower(0, MapNode(func(string) int { t.Fatal("f called on error"); return 0 }, Tag("button", On("click", fail))))
...
// MapNode panics on a nil function, at construction, where the caller is
// on the stack.
func TestMapNodeNilFuncPanics(t *testing.T) {
	defer func() {
		if recover() == nil {
			t.Fatal("expected panic for a nil function")
		}
	}()
	MapNode[string, string](nil, Text("x"))
}
