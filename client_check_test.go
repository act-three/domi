...
// ignores tsc's other complaints about the untyped domi.js.
var undefinedNameCodes = []string{"error TS2304", "error TS2552", "error TS2448"}

// TestClientJSNoUndefinedNames type-checks domi.js with tsc and fails
...
func TestClientJSNoUndefinedNames(t *testing.T) {
	...
	if err != nil {
		t.Skip("tsc not installed; skipping domi.js undefined-name check")
	}
	...
	for _, line := range strings.Split(string(out), "\n") {
		for _, code := range undefinedNameCodes {
			if strings.Contains(line, code) {
				t.Errorf("domi.js: %s", strings.TrimSpace(line))
			}
		}
	}
}

// TestClientJSNoShadowedVars lints domi.js for a helper parameter that
...
func TestClientJSNoShadowedVars(t *testing.T) {
	...
	if err != nil {
		t.Skip("eslint not installed; skipping domi.js shadowing check")
	}
